...
namespace DF.Finance.Common
{
    public class Utils
    {
        ...
        /// <returns>处理后的字符串</returns>
        /// //把HTML代码转换成TXT格式
        public static String ToTxt2(String Input)
        {
            StringBuilder sb = new StringBuilder(Input);
            sb.Replace("&nbsp;", " ");
            sb.Replace("&lt;", "<");
        ...
        public static bool SqlFilter(string sInput)
        {
            if (sInput == null || sInput == "")
                return false;
            string sInput1 = sInput.ToLower();
        ...
        public static Dictionary<string, string> GetRequestPost()
        {
            ...
            {
                sArray.Add(requestItem[i], HttpContext.Current.Request.Form[requestItem[i]].Trim());
            }
            return sArray;
        }
        /// <summary>
        ...
        /// </summary>
        /// <returns></returns>
        public static Dictionary<string, string> GetRequest()
        {
            StreamReader reader = new StreamReader(HttpContext.Current.Request.InputStream);
            string bodyText = reader.ReadToEnd();
            return Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, string>>(bodyText);
        }
        ...
        /// <returns></returns>
        public static  string GetDefaultSetting(string columnName, string dataType, string defaultSetting)
        {
            ...
            defaultSetting = defaultSetting.Replace("(", "").Replace(")", "").Replace("N", "").Replace("'", "").Replace("‘", "").Replace("‘’", "").Replace("‘", "").Replace("’", "");
            defaultSetting = Regex.Replace(defaultSetting, "‘’", "", RegexOptions.IgnoreCase);
            //处理 
            switch (dataType)
            {
                case "string":
                    if (string.IsNullOrEmpty(defaultSetting) || defaultSetting.Trim().Equals(" ‘’") || defaultSetting.IndexOf(" ‘’") > 1)
                    {
                        defaultSetting = "\"\"";
                    }
                    else
                    ...
                    break;
                case "DateTime":
                    if (defaultSetting == "getdate")
                    {
                        defaultSetting = "DateTime.Now";
                    }
                    else
                    {
                        defaultSetting = "\"" + defaultSetting + "\"";
            ...
            return "get{if(" + columnName + "==null){return " + defaultSetting + ";}else{" + columnName + "}};set{" + columnName + " = value;}";
        }
    }
}
