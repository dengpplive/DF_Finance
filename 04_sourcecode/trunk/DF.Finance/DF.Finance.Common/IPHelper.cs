...
namespace DF.Finance.Common
{
    ...
    public static class IPHelper
    {
        ...
        /// <summary>
        /// 判断是否手机打开（true 手机；false 非手机）
        /// </summary>
        /// <returns></returns>
        public static bool IsPhone()
        {
            string u = System.Web.HttpContext.Current.Request.ServerVariables["HTTP_USER_AGENT"];
            Regex b = new Regex(@"iphone|android|symbianos|windows\sphone", RegexOptions.IgnoreCase | RegexOptions.Multiline);
            var result = b.IsMatch(u) || System.Web.HttpContext.Current.Request.UserAgent.ToLower().Contains("micromessenger");
            return result;
        }
        /// <summary>
        /// 客户端是否来自微信浏览器
        ...
        public static string GetIP()
        {
            ...
            if (null == loginip || loginip == String.Empty || !IsIP(loginip))
            {
            ...
            if (loginip == "::1")
            {
                System.Net.IPAddress[] addressList = Dns.GetHostEntry(Dns.GetHostName()).AddressList;//本地ip
                string strIpv4 = "";
                ...
                IPAddress localIp = IPAddress.Parse(strIpv4);
                loginip = localIp.ToString();
            }
            return loginip;
        }
