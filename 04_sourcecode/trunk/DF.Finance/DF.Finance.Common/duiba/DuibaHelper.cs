...
using System.Configuration;//哈希算法必须引用
namespace DF.Finance.Common
{
    ...
    public class DuibaHelper
    {
        ...
        public static string AppKey = ConfigurationSettings.AppSettings["DuibaAppKey"];
        public static string AppSecret = ConfigurationSettings.AppSettings["DuibaAppSecret"];
        /**
         * 构建一个带兑吧签名的url
         * @param url 接口url地址
         ...
         * @return
         */
        public static string BuildUrlWithSign(string url, Hashtable hshTable)
        {
        ...
        * @return
        */
        public static bool SignVerify(Hashtable hshTable)
        {

            string sign = string.Empty;
            hshTable.Add("appSecret", AppSecret);
            LogHelper.WriteLog("服务器AppSecret：" + AppSecret);
            LogHelper.WriteLog("获得sign：" + AppSecret);
            if (hshTable.ContainsKey("sign"))
            {
                sign = hshTable["sign"].ToString().ToLower();
            }
            LogHelper.WriteLog("转换后sign：" + Sign(hshTable));
