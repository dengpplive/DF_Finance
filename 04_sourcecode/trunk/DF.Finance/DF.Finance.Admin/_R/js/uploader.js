$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        water: false, //是否加水印 未实现
        thumbnail: false, //是否生成缩略图 未实现
        sendurl: "/Upload/Upload", //发送地址
        filetypes: "jpg,jpge,png,gif", //文件类型
        filesize: "4048", //文件大小
        btntext: "上传...", //上传按钮的文字
        swf: "~/_R/js/webuploader/uploader.swf" //SWF上传控件相对地址
    };
    //初始化上传控件
        ...
        var fun = function (parentObj) {
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.Code == '1') {
                    //如果是单文件上传，则赋值相应的表单
                    if (!p.multiple) {
                        parentObj.parent().siblings(".upload-name").val(data.Data.name);
                        console.log(parentObj.parent().parent().find(".upload-path"));
                        parentObj.parent().parent().find(".upload-path").val(data.Data.path);
                        parentObj.parent().siblings(".upload-size").val(data.Data.size);
                    } else {

                        addImage(parentObj, data.Data.path, data.Data.thumb);
                        //TODO： 多文件上传暂未实现
                    }
                    var progressObj = parentObj.children(".upload-progress");
...
function addImage(targetObj, originalSrc, thumbSrc) {
    ...
    //插入到相册UL里面
    var newLi = $('<li>'
    + '<input type="hidden" name="hid_photo_name" value="'+originalSrc+'" />' //元数据：+ '<input type="hidden" name="hid_photo_name" value="0|' + originalSrc + '|' + thumbSrc + '" />'
    + '<input type="hidden" name="hid_photo_remark" value="" />'
    + '<div class="img-box" onclick="setFocusImg(this);">'
    + '<a href="' + originalSrc + '" data-gallery=""><img src="' + originalSrc + '" bigsrc="' + thumbSrc + '"  style="width:112px;height:112px"/></a>'
